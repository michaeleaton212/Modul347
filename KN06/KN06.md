...
Wir wollen zeigen wie man Nodes aus dem Cluster entfernt. Das Entfernen geschieht in zwei Schritten: erst meldet sich der Node selbst ab, dann entfernt der Master ihn aus der Liste.

Zuerst wurde Node 3 entfernt:
...
Anschliessend wurde Node 2 ebenfalls entfernt:
...
**Befehl 2 auf Node 1:**
```bash
microk8s remove-node ip-172-31-73-162
```

`microk8s leave` wird auf dem Node ausgefuehrt der den Cluster verlassen soll. Der Node meldet sich sauber beim Cluster ab. `microk8s remove-node` entfernt den angegebenen Node aus der Cluster-Registrierung.

**Ergebnis auf Node 1 nach dem Entfernen beider Nodes:**

![remove-node und get nodes](KN06_B3_remove_node.png)

Node 2 und Node 3 sind aus der Cluster-Liste entfernt. Nur noch Node 1 ist sichtbar.

---

### B4 – Nodes als Worker wieder hinzufuegen

Nicht jeder Node muss ein Master sein. Worker-Nodes sind spezialisiert auf das Ausfuehren von Pods, waehrend der Master-Node die Verwaltungsaufgaben uebernimmt. Wir fuegen Node 2 und Node 3 als reine Worker hinzu.

Fuer jeden Node wurde auf Node 1 zuerst ein neuer Token generiert:
```bash
microk8s add-node
```

Dann auf Node 2 als Worker beigetreten:
```bash
...
Node 2 und Node 3 sind dem Cluster als Worker-Nodes beigetreten. Der Cluster besteht nun aus einem Master-Node (Node 1) und zwei Worker-Nodes (Node 2, Node 3).

---

### B5 – `microk8s status` nach Worker-Join

Nach dem Hinzufuegen von Node 2 und Node 3 als Worker wollen wir sehen wie sich der Status des Clusters veraendert hat.

**Ausgefuehrt auf Node 1 nach Worker-Join:**

![microk8s status nach worker join](KN06_B5_status_nach_worker.png)

| | Vorher (3x Control-Plane) | Nachher (1x Master, 2x Worker) |
|---|---|---|
| `high-availability` | yes | no |
| `datastore master nodes` | alle 3 IPs | nur Node 1 (172.31.76.72) |

High Availability ist auf `no` gefallen weil jetzt nur noch ein Node die Cluster-Datenbank verwaltet. Fuer HA braucht es mindestens 3 Datastore-Master. Die Worker-Nodes haben keinen Zugriff auf die Datenbank und zaehlen daher nicht.
